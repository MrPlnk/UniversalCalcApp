...
class GetMemoryByTypeUseCase @Inject constructor(
    ...
    operator fun invoke(type: Token.Type): Result<OperationMemory?> = runCatching{
        memoryRepository.getMemoryByType(type)
    }

    operator fun invoke(expression: String): Result<OperationMemory?> = runCatching{
